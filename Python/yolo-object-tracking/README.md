...
# Implementation of the Pre-Trained YOLO Model for Automated Object Tracking

___
## Objective
Build an algorithm enabling the automated detection and tracking of objects on image and video files using the weights of the pre-trained deep learning model named YOLO (You Only Look Once).
...
plt.subplot(121)
v0 = cv2.VideoCapture('data/video-samples/' + videos[0])
_, frame = v0.read()
plt.imshow(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
plt.title(videos[0].split('.')[0])
plt.subplot(122)
v1 = cv2.VideoCapture('data/video-samples/' + videos[1])
_, frame = v1.read()
plt.imshow(cv2.cvtColor(frame, cv2.COLOR_BGR2RGB))
...
Let's visualize the output of the img_format function.
...
plt.subplot(122)
plt.imshow(cv2.cvtColor(np.squeeze(img_process(frame), axis=0), cv2.COLOR_BGR2RGB))
...
**Let's try this algorithm on the first frame of the first video sample.**
...
v0 = cv2.VideoCapture('data/video-samples/' + videos[0])
_, frame = v0.read()
print('Test on first frame of: ', videos[0])
apply_yolo_img(frame)
```

    Test on first frame of:  Airport.mp4
...
**We will define a function that applies the same method used in the single frame example to every frame of a video.**

_Note: since the live feed may be significantly slowed down by the prediction process, we will save the results as a separate video file instead of displaying the live predictions frame by frame._
...
def apply_yolo_vid(video, model, list_of_classes):
    ...
    while True:
        ...
        clear_output()
        print('\tProcessing frame ', current_frame, '/', num_frames)
    ...
    print('Total computation time: {0:.2f}s'.format(end - start))
```
